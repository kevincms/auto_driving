...
class MainNode(Node):
    def __init__(self):
        ...
        # light & labacon 변수
        self.check_light = True
        self.labacon_mod = False
        self.labacon_count = 0
    ...
    def main_loop(self):
        if self.check_light:
            color=light.define_light(self)
            if color=="green":
                self.check_light=False
                self.labacon_mod=True   
        elif self.labacon_mod:
            self.labacon_count+=1
            self.speed=7
